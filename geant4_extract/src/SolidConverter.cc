...
static gp_Trsf BuildTransform(
    ...
    trsf.SetValues(
        rot.xx(), rot.xy(), rot.xz(), tx,
        rot.yx(), rot.yy(), rot.yz(), ty,
        rot.zx(), rot.zy(), rot.zz(), tz
    );
    return trsf;
}
...
TopoDS_Shape SolidConverter::Convert(G4VSolid* solid)
{
    ...
    if (auto* cons = dynamic_cast<G4Cons*>(solid)) {
        ...
        if (rmin1 <= 0.0 && rmin2 <= 0.0) return outer;
    ...
    if (auto* sph = dynamic_cast<G4Sphere*>(solid)) {
        ...
        auto make_sphere = [&](double r) -> TopoDS_Shape {
            if (full) return BRepPrimAPI_MakeSphere(ax, r).Shape();
            double a3 = full_phi ? kTwoPi : dphi;
            return BRepPrimAPI_MakeSphere(ax, r, a1, a2, a3).Shape();
        };
        if (rmin <= 0.0) return make_sphere(rmax);
    ...
    std::cout << "Unsupported solid type: "
...
TopoDS_Shape SolidConverter::ConvertViaPolyhedron(G4VSolid* solid)
{
    ...
    if (auto* tess = dynamic_cast<G4TessellatedSolid*>(solid)) {
        ...
        for (int i = 0; i < tess->GetNumberOfFacets(); ++i) {
            ...
            if (!f || f->GetNumberOfVertices() < 3) continue;
            ...
            for (int k = 1; k + 1 < f->GetNumberOfVertices(); ++k) {
                ...
                if (p0.Distance(p1) < 1e-9 ||
                    p1.Distance(p2) < 1e-9 ||
                    p0.Distance(p2) < 1e-9) continue;
                try {
                    BRepBuilderAPI_MakePolygon tri;
                    tri.Add(p0); tri.Add(p1); tri.Add(p2); tri.Close();
                    if (!tri.IsDone()) continue;
    ...
    while (true) {
        ...
        for (int k = 1; k + 1 < nVerts; ++k) {
            ...
            gp_Pnt p2(nodes[k+1].x()/mm, nodes[k+1].y()/mm, nodes[k+1].z()/mm);
            if (p0.Distance(p1) < 1e-9 ||
                p1.Distance(p2) < 1e-9 ||
                p0.Distance(p2) < 1e-9) continue;
            try {
...
TopoDS_Shape SolidConverter::ConvertPolycone(G4Polycone* solid)
{
    ...
    auto addSeg = [&](double r1, double z1, double r2, double z2) {
        if (std::abs(r1-r2) < 1e-12 && std::abs(z1-z2) < 1e-12) return;
    ...
    if (!wb.IsDone()) return TopoDS_Shape();
...
TopoDS_Shape SolidConverter::ConvertSubtraction(G4SubtractionSolid* solid)
{
    ...
    if (left.IsNull()) return TopoDS_Shape();
    if (right.IsNull()) return left;
...
TopoDS_Shape SolidConverter::ConvertIntersection(G4IntersectionSolid* solid)
{
    ...
    common.Build();
    if (!common.IsDone() || common.Shape().IsNull()) return TopoDS_Shape();
    return common.Shape();
}
